...
class CNNDenoiser27(BaseBackbone):
    """
    U-Net denoiser.
    Implemented using 4 blocks/stage × 3 stages = 12 encoder + 3 bottleneck + 12 decoder = 27 Conv blocks
    """
    ...
    def __init__(
        ...
        self.down1d = DownBlock(C * 2, C * 2, time_emb_dim)
        ...
        self.down2d = DownBlock(C * 4, C * 4, time_emb_dim)
        ...
        self.down3d = DownBlock(C * 8, C * 8, time_emb_dim)
        ...
        self.up_block3d = UpBlock(C * 8, C * 8, time_emb_dim)
        ...
        self.up_block2d = UpBlock(C * 4, C * 4, time_emb_dim)
        ...
        self.up_block1d = UpBlock(C * 2, C * 2, time_emb_dim)
    ...
    def forward(self, x: torch.Tensor, cond_emb: torch.Tensor) -> torch.Tensor:
        ...
        s1d = self.down1d(s1c, cond_emb)

        s2a = self.down2a(self.pool1(s1d), cond_emb)
        s2b = self.down2b(s2a, cond_emb)
        s2c = self.down2c(s2b, cond_emb)
        s2d = self.down2d(s2c, cond_emb)

        s3a = self.down3a(self.pool2(s2d), cond_emb)
        s3b = self.down3b(s3a, cond_emb)
        s3c = self.down3c(s3b, cond_emb)
        s3d = self.down3d(s3c, cond_emb)

        # Bottleneck (block -> attn -> block -> attn -> block)
        b = self.bottleneck1(self.pool3(s3d), cond_emb)
        b = self.attn_bot1(b)
        ...
        u = self.up_block3d(self.up3(b), s3d, cond_emb)
        u = self.up_block3c(u, s3c, cond_emb)
        u = self.up_block3b(u, s3b, cond_emb)
        u = self.attn_8x8(u)
        u = self.up_block3a(u, s3a, cond_emb)

        u = self.up_block2d(self.up2(u), s2d, cond_emb)
        u = self.up_block2c(u, s2c, cond_emb)
        u = self.up_block2b(u, s2b, cond_emb)
        u = self.attn_16x16(u)
        u = self.up_block2a(u, s2a, cond_emb)

        u = self.up_block1d(self.up1(u), s1d, cond_emb)
        u = self.up_block1c(u, s1c, cond_emb)
