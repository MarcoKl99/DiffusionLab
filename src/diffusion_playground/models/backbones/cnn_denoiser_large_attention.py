...
class CNNDenoiserLargeAttention(BaseBackbone):
    """
    CNNDenoiserLarge extended with self-attention blocks at low spatial resolutions.

    Identical to CNNDenoiserLarge except for two AttentionBlocks inserted after the
    bottleneck (4×4) and after the first decoder block (8×8). At these resolutions the
    O(N²) attention cost is negligible, while the global context helps the model reason
    about coherent structure across the whole image.
    """
    ...
    def forward(self, x: torch.Tensor, cond_emb: torch.Tensor) -> torch.Tensor:
        ...
        bottleneck = self.attn_bottleneck(bottleneck)

        # Decoder + Skip-Connections
        up1 = self.up1(bottleneck)
        up1 = self.up_block1(up1, x3, cond_emb)
        up1 = self.attn_8x8(up1)
